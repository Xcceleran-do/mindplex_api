...
export function toNames(input: { name: string } | { name: string }[]) {
  ...
  return Array.isArray(input) ? input.map((item) => item?.name ?? item).join(",") : (input.name ?? input);
}
 ...
 * ```
 */
export function createFieldsSchema<T extends PgTable>(table: T, forbiddenFields: Set<string>) {
 ...
 * ```
 */
export function getAllowedFields<T extends PgTable>(table: T, forbiddenFields: Set<string>): string[] {
  const allColumns = getColumns(table);
 ...
 * ```
 */
export function sanitizeUpdates<T extends Record<string, any>>(updates: T, allowedFields: Set<string>): Partial<T> {
...
export function buildRelationalWith<TableName extends keyof DbClient["query"]>(
  ...
  const relationalWith = rawIncludes.reduce(
    (acc, inc) => {
