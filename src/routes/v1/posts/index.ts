...
// app.get('/', guard("optional"), postListDocs, validator('query', PostSchema), async (c) => {
//     const db = c.get("db")
//     const { fields, limit, page, include = [] } = c.req.valid("query")
...
// app.patch('/:id', validator('param', PostIdParamSchema), validator('query', PostDetailsSchema), async (c) => {
//     const db = c.get("db")
//     const { fields, include = [] } = c.req.valid("query")
//     const { id } = c.req.valid("param")
