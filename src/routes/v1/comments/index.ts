...
app.get("/:id/replies", guard("optional"), listRepliesDocs, validator("param", CommentIdParamSchema), validator("query", ReplyListQuerySchema), async (c) => {
    const db = c.get("db");
...
// PATCH /comments/:id
app.patch("/:id", guard("user"), updateCommentDocs, validator("param", CommentIdParamSchema), validator("json", UpdateCommentSchema), async (c) => {
    const db = c.get("db");
    const { id } = c.req.valid("param");
