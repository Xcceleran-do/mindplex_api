...
const FIRST_NAMES = [
    ...
    "Kai", "Rowan", "Dakota", "Phoenix", "Skyler",
    "Finley", "Harley", "Reese", "Blair", "Drew",
];

const LAST_NAMES = [
    "Chen", "Patel", "Kim", "Santos", "Okafor",
    "Mueller", "Silva", "Tanaka", "Johansson", "Ali",
    "Nakamura", "Petrov", "Andersen", "Morales", "Ibrahim",
    "Kowalski", "Fischer", "Yamamoto", "Nguyen", "Park",
];
...
const TAXONOMY_TAGS = [
    ...
    "ethics", "governance", "decentralization", "web3",
    "privacy", "security", "open-source", "research",
    "opinion", "tutorial", "news-analysis", "podcast-notes",
];

const FAQ_ITEMS = [
    { q: "How do I create an account?", a: "Click the Sign Up button in the top right corner and follow the registration steps." },
    { q: "How does the reputation system work?", a: "Your reputation is calculated from your interactions including posts, comments, and community votes." },
    { q: "Can I connect my crypto wallet?", a: "Yes, go to Settings → Wallet and connect your wallet using the supported providers." },
    { q: "How do I reset my password?", a: "Click Forgot Password on the login page and follow the email instructions." },
    { q: "What are People's Choice votes?", a: "Community members can vote on articles they think deserve special recognition." },
    { q: "How do polls work?", a: "Editors can attach polls to articles. You get one vote per poll unless it's marked as multiple choice." },
];

const INTERESTS = [
    "Artificial Intelligence", "Machine Learning", "Blockchain",
    "Neuroscience", "Philosophy of Mind", "Cryptography",
    "Data Science", "Robotics", "Quantum Computing",
    "Ethics", "Cognitive Science", "Web Development",
];

const EDUCATION_OPTIONS = [
    "BSc Computer Science", "MSc Artificial Intelligence", "PhD Neuroscience",
    "BA Philosophy", "MSc Data Science", "Self-taught Developer",
    "BSc Mathematics", "MBA Technology Management", "PhD Computational Linguistics",
];
...
async function seedUserProfiles(users: { id: number }[]) {
    ...
    const profiles = users.map((u, i) => ({
        ...
        bio: i % 3 === 0 ? "AI researcher and decentralization enthusiast." : i % 3 === 1 ? "Building the future of community-driven knowledge." : "Exploring the intersection of technology and society.",
...
async function seedWallets(users: { id: number }[]) {
    ...
    const wallets = walletUsers.map((u) => ({
        ...
        paymentAddress: Math.random() > 0.5
            ? `addr1_${Buffer.from(crypto.getRandomValues(new Uint8Array(16))).toString("hex")}`
            : null,
...
async function seedPostAuthors(
...
async function seedPostTaxonomies(
    posts: { id: number }[],
...
async function seedCommentClassifications(
    comments: { id: number }[],
    users: { id: number }[]
) {
...
async function seedSocialInteractions(
    ...
    console.log(`    → ${emojiRecords.length} emojis, ${bookmarkRecords.length} bookmarks, ${shareRecords.length} shares`);
}
...
async function seedPolls(posts: { id: number }[], users: { id: number }[]) {
    ...
    for (let i = 0; i < pollData.length; i++) {
        ...
        const options = await db.insert(schema.pollOptions).values(
...
async function seedFaqs() {
    ...
    const [advancedCat] = await db.insert(schema.faqCategories).values({
...
async function seedContentSources(users: { id: number }[]) {
    ...
    const sources = [
        { name: "ArXiv AI Papers", url: "https://arxiv.org/list/cs.AI/recent", type: "rss" },
        { name: "Hacker News", url: "https://news.ycombinator.com/rss", type: "rss" },
        { name: "AI Blog Aggregator", url: "https://example.com/ai-blogs", type: "api" },
    ];
...
async function seedMailings(users: { id: number }[]) {
    ...
    // Contact submissions
    const contacts = [
        { firstName: "Test", lastName: "User", email: "test@example.com", message: "Love the platform! When is the mobile app coming?" },
        { firstName: "Bug", lastName: "Reporter", email: "bugs@example.com", message: "Found an issue with the comment threading on mobile Safari." },
    ];
    ...
    console.log(`    → ${subscribers.length} subscribers, ${contacts.length} contacts, 1 campaign with ${deliveries.length} deliveries`);
}
